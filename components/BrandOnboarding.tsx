...
/*
 * BrandOnboarding (Phase 1.5A — Guided setup)
 * A step-by-step onboarding wizard shown to NEW users (no saved brand profile).
 * It replaces the long Profil Brand form with simple questions and selectable
 * suggestions, then saves the SAME BrandSnapshot structure to localStorage so
 * the existing generator and tools keep working unchanged.
 *
 * It does NOT change the data model, the generator, or any core function.
 * Existing users with a saved profile never see this (see BrandSetupClient).
 */
...
export default function BrandOnboarding({
  ...
  function next() {
    ...
    if (step === 1 && (!businessName.trim() || !area.trim())) {
      setErr('Isi nama salon dan kota/wilayah layanan dulu ya.');
      return;
    }
        ...
        <section>
          <div className="card">
            <div className="ob-success">
              ...
              <h2>Profil salon siap!</h2>
              <p className="notion-muted">
                Profil <strong>{businessName.trim() || 'salon kamu'}</strong> sudah
  ...
  return (
    <>
      <section className="page-head">
        ...
        <h1>Yuk siapkan profil salonmu</h1>
        <p>
      ...
      <section>
        <div className="card">
          ...
          {step === 1 && (
            <div>
              <h3 style={headStyle}>1. Identitas Salon</h3>
              <p className="notion-muted" style={leadStyle}>
                Info dasar yang akan tampil di konten.
              </p>
              <div className="form-grid">
                <div className="field">
                  <label>Nama salon <span className="req">*</span></label>
                  <input
                    type="text"
                    value={businessName}
          ...
          <div className="btn-row" style={navRowStyle}>
            ...
            <Button onClick={next}>
              {step === TOTAL_STEPS ? 'Selesai & Simpan ✅' : 'Lanjut →'}
            </Button>
            {onUseManualForm ? (
